...
func testProgressBar() {
	...
	// Print instructions
	layer1.Locate(0, 0)
	layer1.Print("Press 'h' to increment horizontal bar")
	layer1.Locate(0, 1)
	layer1.Print("Press 'v' to increment vertical bar")
	layer1.Locate(0, 2)
	layer1.Print("Press 'q' to quit")
...
func testScrollBars() {
	...
	for {
		...
		layer1.Print("  ")
		layer1.Locate(10, 0)
		layer1.Print(strconv.Itoa(s1.getScrollValue()))
		layer1.Locate(10, 1)
		layer1.Print("  ")
		layer1.Locate(10, 1)
...
func testSelector() {
	...
	layer1.Locate(3, 3)
	layer1.Print(menuBarInstance.layerAlias)
	for {
		UpdateDisplay(false)
		time.Sleep(50 * time.Millisecond)
		layer1.Locate(3, 3)
...
func testWindowMovement() {
	...
	layer1.Print("This is a test of the layer system")
	layer1.Locate(1, 1)
	layer1.Print(strconv.Itoa(Layers.Get(layer2.layerAlias).ZOrder))
	styleEntry := types.NewTuiStyleEntry()
...
func testButtonPressAction() {
	...
	for {
		...
		characterEntry := getCellInformationUnderMouseCursor(mouseXLocation, mouseYLocation)
		layer1.Locate(0, 0)
		layer1.Print(fmt.Sprintf("%d, %d, %d   ", characterEntry.AttributeEntry.CellType, characterEntry.AttributeEntry.CellControlId, characterEntry.AttributeEntry.CellControlLocation))
		UpdateDisplay(false)
		key := string(Inkey())
		if key == "q" {
			break
