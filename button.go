...
func (shared *buttonType) drawButtonsOnLayer(layerEntry types.LayerEntryType) {
	...
	for _, buttonEntry := range buttons {
		drawButton(&layerEntry, buttonEntry.ButtonAlias, buttonEntry.ButtonLabel, buttonEntry.StyleEntry, buttonEntry.IsPressed, buttonEntry.IsSelected, buttonEntry.IsEnabled, buttonEntry.XLocation, buttonEntry.YLocation, buttonEntry.Width, buttonEntry.Height)
	}
}
	...
	if characterEntry.AttributeEntry.CellType != constants.CellTypeButton {
		...
		for currentLayer := range memory.Buttons.MemoryManager {
			...
			for _, buttonEntry := range buttons {
				...
				if !memory.IsButtonExists(currentLayer, buttonEntry.ButtonAlias) {
					continue
				}
