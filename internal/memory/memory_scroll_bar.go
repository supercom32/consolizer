...
func DeleteAllScrollbarsFromLayer(layerAlias string) {
	...
	for _, scrollbar := range scrollbars {
		ScrollBars.Remove(layerAlias, scrollbar.Alias) // Assuming scrollbar.Alias contains the alias
	}
}

func compareScrollbarAlias(a, b *types.ScrollbarEntryType) bool {
	return a.Alias < b.Alias // Compare lexicographically in ascending order
}
